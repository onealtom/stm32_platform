...
UCHAR8 SaveInitData(UCHAR8 npcs,UINT16 current_p, UCHAR8  total_p,UCHAR8 * p_data,UINT16 len)
{
	...
	{	
		...
		FlashWrite( page,0, p_data, wr_len,1);	// Ð´ÈëÊý¾Ý
	...
	//if(cmd_cnt<250)
	{
		...
		{	
			...
			wr_len = (len>FLASH_PAGE_SIZE) ? FLASH_PAGE_SIZE : len;
			FlashWrite( page,0,p_init, wr_len,1);	// Ð´ÈëÊý¾Ý

			page++;
			p_init += wr_len;		// Ö¸ÕëºóÒÆ
			len -= wr_len;		// ¼ÆËãÊ£Óà³¤¶È						
		}
		//FlashWriteWithinpage( page,0, (UCHAR8 *)p_init_data, sizeof(SYSTEM_INIT_DATA_T),1);	// Ð´ÈëÊý¾Ý
...
Description:´ÓFLASHÖÐ»ñÈ¡³õÊ¼»¯ÐòÁÐ£¬²¢³õÊ¼»¯ÊÕ·¢Æ÷
Input:A,B
Output:void         
Return:<0 ´íÎó£¬0:³É¹¦
**************************************************************/
...
//	}
	...
	{
		//Êý¾Ý³¤¶È,´ÓÃüÁî×Ü¸öÊý(2byte)¿ªÊ¼£¬---Ã¿¸öÃüÁî
		...
		while( len )
		{
			...
			count = 0;
			cmd_cnt = (data[0]<<8)|data[1];	//°üÖÐ°üº¬µÄÃüÁî¸öÊý
...
void SetAtt3(UCHAR8 abcd,UCHAR8 att)
{
	...
	//075[D6~D1]ÖÐÐ´ÈëÏàÓ¦µÄË¥¼õÖµ
	att <<= 1;
	att &= 0x7e;
	ReadWriteTF(npcs,1,0x0075,att);

	//7C[D6]Ð´1À´¸üÐÂË¥¼õÖµÊ¹ÆäÉúÐ§¡£
...
UCHAR8 SetAtt3Tx(UCHAR8 npcs,UCHAR8 att)
{
	...
	ReadWriteTF(npcs,1,0x169,0xCC);  //¹Ø±ÕRXÊµÊ±Õý½»Ð£×¼£»
	ReadWriteTF(npcs,1,0x18B,0x8D);   //¹Ø±ÕRX BBDCÊµÊ±Ð£×¼£»
	ReadWriteTF(npcs,1,0x014,0x03);    //Ð¾Æ¬½øÈëµÈ´ýÄ£Ê½£¨´ËÊ±Ð¾Æ¬ÎÞÊä³ö£©
	ReadWriteTF(npcs,1,0x0AC,att);   //½«¸Õ²ÅµÄATT3ÖµÐ´Èë£¨ÕâÀïÎª0.5dB²½½ø£©£¨±ÈÈçATT3ÉèÖÃÎª10dB£©
	ReadWriteTF(npcs,1,0x016,0x10);   // ¿ªÊ¼TXÕý½»Ð£×¼£»
	ReadWriteTF(npcs,1,0x169,0xCE);   //Æô¶¯RXÊµÊ±Õý½»Ð£×¼£»
	ReadWriteTF(npcs,1,0x18B,0xAD);   //Æô¶¯RX BBDCÊµÊ±Ð£×¼£»
	ReadWriteTF(npcs,1,0x014,0x23);    //Ð¾Æ¬½øÈëÕý³£¹¤×÷×´Ì¬¡£
...
void TransSetPll(UCHAR8 up_down_flag, FLOAT32 pll)
{
	...
	ReadWriteTF(up_down_flag?TF_B:TF_A,1,0x231,(UCHAR8)(pll_int));			// Write Rx Synth Integer Freq Word[7:0] Í¬Ê±Æô¶¯VCOÐ£×¼
	...
	ReadWriteTF(up_down_flag?TF_A:TF_B,1,0x271,(UCHAR8)(pll_int));			// Write Tx Synth Integer Freq Word[7:0] Í¬Ê±Æô¶¯VCOÐ£×¼
