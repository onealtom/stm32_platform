...
int uart_write_bytes(int port , char * buf, int len)
{

}
#define portTICK_RATE_MS 1
int uart_read_bytes(int port , char * buf, int len, int timeout)
{
...
int ymodem_start_pkt(uint8_t *data, char *fileName, uint32_t length)
{
	int i, err;
	unsigned char receivedC;
	...
	{
		// Send Packet
	uart_write_bytes(EX_UART_NUM, (char *)data, PACKET_SIZE + PACKET_OVERHEAD);
...
int ymodem_mid_pkt(uint8_t * src_data, int pktNo, uint32_t sizeBlk  )
{
	uint32_t size = 0;
	int i;
	static uint16_t blkNumber;
	int err;
	uint8_t *p;
	uint16_t tempCRC;
	
/* package data */
	p = src_data -3;  //使用原来的内存空间, 避免memcpy
	p[0] = STX;
	p[1] = (pktNo & 0x000000ff);
	p[2] = (~(pktNo & 0x000000ff));

	size = sizeBlk < PACKET_1K_SIZE ? sizeBlk :PACKET_1K_SIZE;

	if ( size  < PACKET_1K_SIZE) {
		for (i = size + PACKET_HEADER; i < PACKET_1K_SIZE + PACKET_HEADER; i++) {
			p[i] = 0x00; // EOF (0x1A) or 0x00
		}
	}
	tempCRC = crc16(&p[PACKET_HEADER], PACKET_1K_SIZE);
	p[PACKET_1K_SIZE + PACKET_HEADER] = tempCRC >> 8;
	p[PACKET_1K_SIZE + PACKET_HEADER + 1] = tempCRC & 0xFF;
/* package data  end*/
	
	uart_write_bytes(EX_UART_NUM, (char *)p, PACKET_1K_SIZE + PACKET_OVERHEAD);
...
//int Ymodem_FiberTransmit (char* sendFileName, unsigned int sizeFile, FILE *ffd)
int Ymodem_StreamFile_TX( uint16_t msg_length, uint8_t * p_msg_dat, uint8_t * p_tx_buff )
{
	uint8_t packet_data[PACKET_1K_SIZE + PACKET_OVERHEAD];
	uint16_t blkNumber;
	unsigned char receivedC;
	...
	{
		// Prepare and send next packet
		Ymodem_PreparePacket(packet_data, blkNumber, size, 0);
		do
		{
			uart_write_bytes(EX_UART_NUM, (char *)packet_data, PACKET_1K_SIZE + PACKET_OVERHEAD);
