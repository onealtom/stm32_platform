...
#ifndef _PL_SPI_DRV_H_
#define _PL_SPI_DRV_H_
...
/*no use*///commonErr_t CMB_closeHardware(void);

/* GPIO function */
/*no use*///commonErr_t CMB_setGPIO(uint32_t GPIO);

/* hardware reset function */
commonErr_t CMB_hardReset(uint8_t spiChipSelectIndex);

/* SPI read/write functions */
/*no use*///commonErr_t CMB_setSPIOptions(spiSettings_t *spiSettings); /* allows the platform HAL to work with devices with various SPI settings */
/*no use*///commonErr_t CMB_setSPIChannel(uint16_t chipSelectIndex );  /* value of 0 deasserts all chip selects */
...
/*no use*///commonErr_t CMB_openLog(const char *filename);
/*no use*///commonErr_t CMB_closeLog(void);
commonErr_t CMB_writeToLog(ADI_LOGLEVEL level, uint8_t deviceIndex, uint32_t errorCode, const char *comment);
/*no use*///commonErr_t CMB_flushLog(void);

/* platform FPGA AXI register read/write functions */
/*no use*///commonErr_t CMB_regRead(uint32_t offset, uint32_t *data);
/*no use*///commonErr_t CMB_regWrite(uint32_t offset, uint32_t data);

/* platform DDR3 memory read/write functions */
/*no use*///commonErr_t CMB_memRead(uint32_t offset, uint32_t *data, uint32_t len);
/*no use*///commonErr_t CMB_memWrite(uint32_t offset, uint32_t *data, uint32_t len);
